...
def send_user_mail(login, password, priv_key, pub_key, server_pub_key, allowed_ip, dest_email, smtp_password):
    """Construit et envoie l'email avec le mot de passe passé en argument."""
    msg = EmailMessage()
    ...
    msg['Subject'] = f"Plateforme CLUZ - Vos accès personnels : {login}"  # ← modifié
...
def main():
    ...
    try:
        with open(USER_SOURCE_FILE, mode='r', encoding='utf-8') as f:
            ...
            required_cols = ['login', 'password', 'email', 'private_key', 'public_key', 'server_public_key', 'allowed_ip']
